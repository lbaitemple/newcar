...
def getRange(data,angle):
    carAngle = 4*(angle+45)
    if carAngle > 1079:
        carAngle = int(carAngle-1079)

    return data.ranges[int(carAngle)]
...
#	print "In range", values
	...
	rospy.loginfo("Num Points %d", num_points)

	if num_points < 3:
		rospy.loginfo("Go go go - clear path")
		return -1,-1
	#Detects if there is an obstacle	
	elif num_points <= 15:
		rospy.loginfo("normal obstacle")
		k =-1
		for i in values:
			k = k + 1
			if i<= (distance):
		...
		if start_point <= (start+1):
			rospy.loginfo("Start point extended")
			start1 = start - 10
			end1 = start
			start_point = start1
...
def callback(data):
	...
	global flag_left
        ...
        distance=rospy.get_param('~distance', 2.0)
#	a = getRange(data,50)
#	b = getRange(data,0)
#	# c = getRange(data,40)
#	swing = math.radians(50)
...
#		flag_obstacle = 1
	...
	print "new code", final_direction, start_point,end_point
